...
export default function Carro({ filtro }: { filtro: string}) {
                ...
                if(!carro.destaque) {
                    return (
                        <a href="" onClick={(e) => {e.preventDefault(); abrirModal(carro)}} key={carro.id}>
                            <div className="text-zinc-100 w-max p-1 bg-zinc-200 shadow shadow-zinc-50 rounded-lg">
                                <Image src={carro.urlImagem} alt="" width={220} height={1} className="rounded-lg" style={{ height: "140px" }}/>
